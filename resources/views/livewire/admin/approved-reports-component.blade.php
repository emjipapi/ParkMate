            ...
            <tr>
                ...
                <td class="px-3 py-2 text-sm text-gray-700">
                    @if($violation->created_at)
                        <span title="Submitted on: {{ $violation->created_at->toDayDateTimeString() }}@if($violation->updated_at && $violation->updated_at != $violation->created_at)&#10;Approved on: {{ $violation->updated_at->toDayDateTimeString() }}@endif">
                ...
                <td>
                    <div class="d-flex flex-column gap-1" style="max-width: 220px;">
                        <div>
                            <input type="file" wire:model="proofs.{{ $violation->id }}" class="form-control form-control-sm" accept="image/*" />
                            <div wire:loading wire:target="proofs.{{ $violation->id }}" class="mt-2 text-muted">
                                <div class="spinner-border spinner-border-sm text-primary" role="status"></div>
                                Uploading image…
                            </div>
                            @error('proofs.' . $violation->id) <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>

                        <select class="form-select form-select-sm mb-1" wire:model="violationsActionTaken.{{ $violation->id }}" @if($violation->status === 'resolved') disabled @endif>
                            <option value="">Select action</option>
                            <option value="Warning">Warning Issued</option>
                            <option value="6 Months Penalty">6 Months Penalty</option>
                            <option value="Access Denied">Access Denied</option>
                        </select>
                        @error('violationsActionTaken.' . $violation->id) <span class="text-danger small">{{ $message }}</span> @enderror

                        @if($violation->status === 'resolved')
                            <button class="btn btn-sm btn-secondary" disabled>✓ Resolved</button>
                        @else
                            <button wire:click="markForEndorsement({{ $violation->id }})" wire:loading.attr="disabled" class="btn btn-sm btn-primary" wire:target="markForEndorsement({{ $violation->id }})">
    ...
    <article class="bg-white border rounded p-3 shadow-sm mb-3">
        ...
        <div class="small">
            ...
            <div class="d-flex mb-2">
                ...
                <div class="text-dark">
                    @if($violation->violator) {{ trim($violation->violator->firstname . ' ' . $violation->violator->lastname) }} @else N/A @endif
                </div>
            </div>
            ...
            <div class="d-flex mb-2">
                ...
                <div class="text-dark text-break" title="{{ $violation->description }}">{{ Str::limit($violation->description, 80) }}</div>
            </div>
            ...
            <div class="d-flex mb-2">
                ...
                <div class="text-dark">
                    ...
                    <div class="d-flex flex-column gap-1">
                        @if($reportedUrl)
                            <a href="{{ $reportedUrl }}" target="_blank" class="text-decoration-underline text-primary small">View Reported</a>
                        @else
                            <span class="text-muted small">Reported N/A</span>
                        @endif

                        @if($approvedUrl)
                            <a href="{{ $approvedUrl }}" target="_blank" class="text-decoration-underline text-primary small">View Approval</a>
                        @else
        ...
        <footer class="mt-3 pt-3 border-top">
            <div class="d-flex flex-column gap-2">
                ...
                <div>
                    <select class="form-select form-select-sm" wire:model="violationsActionTaken.{{ $violation->id }}" @if($violation->status === 'resolved') disabled @endif>
                        <option value="">Select action</option>
                        <option value="Warning">Warning Issued</option>
                        <option value="6 Months Penalty">6 Months Penalty</option>
                        <option value="Access Denied">Access Denied</option>
                ...
                <div>
                    ...
                    @else
                        <button wire:click="markForEndorsement({{ $violation->id }})" wire:loading.attr="disabled" class="btn btn-sm btn-primary w-100" wire:target="markForEndorsement({{ $violation->id }})">
                            Mark as For Endorsement
                        </button>
                    @endif
                </div>
