...
# Moved constants
AREA_ID = 2
API_URL = 'http://192.168.1.199:8000/api/rfid-area'

def start_server():
    """Start TCP server and wait for client. - Talipapa"""
    while True:
        try:
            ...
            server.listen(1)
            print(f"✅ Talipapa TCP Server started on {HOST}:{PORT}, waiting for RFID module connection...")
            ...
            if b'\x02' in data and b'\x03' in data:
                ...
                try:
                    ...
                    if epc and (epc not in last_seen or (now - last_seen[epc]) > COOLDOWN):
                        ...
                        last_seen[epc] = now
                        try:
                            requests.post(API_URL, json={'epc': epc, 'area_id': AREA_ID})
                        except:
